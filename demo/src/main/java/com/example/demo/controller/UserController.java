...
// tóm lại, Jackson tự động gọi các getter như getCode(), getMessage(), và getResult() để tạo ra các trường trong JSON mà không cần phải tự tạo định dạng đó.
    ...
    //get đtg
    @GetMapping("/{userId}")
    User getUserr(@PathVariable("userId") String id){// phải đủ biến ("userId") ko thì lỗi
    ...
    //cập nhật
    @PutMapping("/{userId}")
    User updateUserr(@PathVariable("userId") String id, @RequestBody UserCreationRequest request){
